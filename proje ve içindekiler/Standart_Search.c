...
void Insert_Priority_Queue_A_Star (Node *const child, Queue **frontier)
{
	...
	if (Empty(*frontier)){
		...
		float f_n_child=child->path_cost + child->state.h_n;
		float f_n_frontier = (*frontier)->node->path_cost + (*frontier)->node->state.h_n; 
		if(f_n_child < f_n_frontier) { // Child has lowest cost			
			new_queue->next = *frontier;
			*frontier new_queue;
			...
			for (temp_queue = *frontier; temp_queue->next!= NULL; temp_queue = temp_queue->next) { 
				f_n_frontier = temp_queue->next->node->path_cost + temp_queue->next->node->state.h_n;
	 			if(F_n_child f_n_frontier){
					new_queue->next = temp_queue->next; temp_queue->next = new_queue;
					return;
		}
	}//If child has highest cost 
